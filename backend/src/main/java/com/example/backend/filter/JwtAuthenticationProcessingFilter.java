...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (request.getRequestURI().equals(NO_CHECK_URL)) { // "/login" 요청
    ...
    /**
     * [리프레시 토큰으로 유저 정보 찾기 & 액세스 토큰/리프레시 토큰 재발급 메소드] 파라미터로 들어온 헤더에서 추출한 리프레시 토큰으로 DB에서 유저를 찾고, 해당
        ...
        usersRepository.findByToken(refreshToken) // 리프레쉬 토큰으로 유저 찾음
            .ifPresent(users -> {
                jwtService.sendAccessAndRefreshToken(response,
                    jwtService.createAccessToken(users.getEmail(), users.getNickname(),
                        users.getRole().getKey()),
    ...
    private String reIssueRefreshToken(Users users) {
        String reIssuedRefreshToken = jwtService.createRefreshToken(); // 리프레쉬 토큰 생성
        users.updateRefreshToken(reIssuedRefreshToken); // 유저 리프레쉬 토큰 업데이트
        usersRepository.saveAndFlush(users); // rp 저장 하고 적용
